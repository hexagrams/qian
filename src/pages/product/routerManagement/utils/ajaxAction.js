...
async function handleGetDataList() {
  ...
  const response = await window.sysAjax.get('/api/productRouter/productRouterList', {
    params: {
      productCode,
      pageSize: 10,
...
async function handleAddDataList(request) {
  const response = await window.sysAjax.post('/api/productRouter/productRouterList', request);
  if (response.code !== 200) {
    return;
  }
...
async function handleDeleteDataItem(id) {
  const response = await window.sysAjax.delete(`/api/productRouter/productRouterList/${id}`);
  if (response.code !== 200) {
    return;
  }
